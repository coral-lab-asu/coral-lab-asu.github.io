...
<style>
  .people-grid.single-center {
    ...
    justify-items: center;      /* center the lone card */
  }
</style>

<!-- ===================== Alumni Panel (text-only, list style) ===================== -->
<div id="alumni-panel" class="people-panel">  <!-- removed `hidden` -->
  <section id="alumni" class="alumni-list-section">
    <style>
      /* Scoped to this panel */
      #alumni.alumni-list-section { padding: .5rem 0 1rem; }
      #alumni .alumni-title { font-size: clamp(1.6rem, 2vw, 2rem); font-weight: 800; margin: 0 0 .75rem; }
      #alumni .alumni-sub { font-size: 1.05rem; color: #555; margin: 0 0 1rem; }
...
/* Center the single faculty card */
.people-grid--faculty{
  /* use the SAME card width as students */
  grid-template-columns: repeat(1, minmax(220px, 280px)); /* match your student grid */
  justify-content: center;     /* center the column */
  justify-items: center;       /* center the card inside */
}

/* Ensure the faculty card + photo use student sizing */
.people-grid--faculty .person-card{
  width: 100%;
  max-width: 280px;            /* same max as students */
}

.people-grid--faculty .person-photo{
  width: 100%;
  aspect-ratio: 1 / 1;         /* square like students */
  object-fit: cover;
}
...
.person-photo{
  width:100%;
  aspect-ratio: 4 / 5;           /* portrait crop like the ref site */
  background:#f6f6f6;
  overflow:hidden;
}
.person-photo img{
...
.person-card:hover .person-name{ color:#8c1d40; }  /* ASU Maroon on hover */
.person-role{
